...
RE_BLAME = re.compile(r'''\^*([0-9a-f]{4,})\s+
                          .+?\s+
                          \(
                          (.+?)\s+
...
MAX_DATE = Timestamp('2105-11-22 14:11:54 -0700')
...
def plot_loc_date(ax, label, history, n_peaks=N_PEAKS):
    ...
    # Mark the peaks
    x0 = tsm.index[0]
    for i, idx in enumerate(peak_idx[:n_peaks]):
        x, y = tsm.index[idx], tsm.iloc[idx]
        x_d, y_d = delta_days(x0, x) + X0, y
        ax.annotate('%d) %s, %.1f' % (i + 1, date_str(x), y),
                    xy=(x_d, y_d), xytext=(x_d, y_d + DY),
                    arrowprops={'facecolor': 'red', 'arrowstyle': 'fancy'},
                    horizontalalignment='center', verticalalignment='center')
        # print('###', x, delta_days(x, x0), y)
        assert X0 <= x_d <= X1, (X0, x_s, X1)


def plot_show(ax, report_map, do_show, graph_path):
    """Show and/or save the current markings in axes `ax`
    """
    summary = report_map.blame_map.git_summary
...
def update_blame_map(blame_map, file_list):
    ...
    file_list = [path for path in file_list if path not in path_hash_loc]
    ...
    for i, path in enumerate(file_list):
        if i % 100 == 0:
            ...
            loc_list = [sum(hash_loc.values()) for hash_loc in path_hash_loc.values()]
            # !@#$ Change this to loc read in this run
            loc = sum(loc_list)
            if loc != last_loc:
                rate = blamed / duration if duration >= 1.0 else 0
                print('i=%d of %d(%.1f%%),files=%d,loc=%d,commits=%d,dt=%.1f,r=%.1f,path=%s' % (
                      i, n_files, 100 * i / n_files, blamed, loc, len(hash_date_author),
                      duration, rate,
                      printable(path)))
                sys.stdout.flush()
                last_loc = loc
...
def filter_blame(blame_map, file_list=None, author_list=None, ext_list=None):
    """
    ...
    print('filter_blame: hash_date_author=%d,path_hash_loc=%d' % (
          len(path_hash_loc),
          len(path_hash_loc)))
    print('file_list,author_list,ext_list=%s' %
          [none_len(o) for o in file_list, author_list, ext_list])
    ...
    print('filter_blame: path_hash_loc=%d' % (len(path_hash_loc)))
    ...
    print('filter_blame: path_hash_loc=%d ext_list=%s' % (len(path_hash_loc), ext_list))
    ...
    hash_date_author = {hsh: date_author for hsh, date_author in hash_date_author.items()
                        if hsh in hashes}
    print('filter_blame: hash_date_author=%d' % len(hash_date_author))

    blame_map.catalog['hash_date_author'] = hash_date_author
    blame_map.catalog['path_hash_loc'] = path_hash_loc
    return blame_map


class ReportMap(object):

    def __init__(self, blame_map, path_list, reports_dir, file_list, author_list, ext_list):
        self.blame_map = filter_blame(blame_map, file_list, author_list, ext_list)
...
def save_tables(report_map):
    print('save_tables')
    blame_map = report_map.blame_map
...
def analyze_blame(report_map, author_list=None, ext_list=None):
    ...
    print('analyze_blame: 1b: hash_loc=%d' % len(hash_loc))
    ...
    if False:
        ...
        for author in sorted(author_loc.keys(), key=lambda k: -author_loc[k]):
            ...
            print('-' * 80)
            print('%s: %d commits, %d loc' % (author, len(date_hash_loc), sum(loc for _, _,loc in date_hash_loc)))
    ...
    if True:
        ...
        author_date_loc = {
            author: [(hash_date_author[hsh][0], hash_loc[hsh]) for hsh in hash_loc.keys()
                     if hash_date_author[hsh][1] == author]
...
def plot_history(report_map,
    ...
    plot_show(ax0, report_map, do_show, graph_path)
...
def main(path_list, force, author_list, ext_list, do_save, do_show):
...
if __name__ == '__main__':
    ...
    main(args, options.force, author_list, ext_list, do_save, do_show)
    print('DONE')
